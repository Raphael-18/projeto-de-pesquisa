...
def Calibracao(
    ...
    # As incrementais são necessárias para averiguar como as vazões obtidas com os parâmetros calibrados
    # adequam-se aos dados "observados" (também advindos de uma calibração própria, devido à parcela de despacho).
    newQ = DownstreamFORK(solution[5], solution[6],
                          solution[7], 24.0, obsAtibaia.Q)

    incValinhos = [0] * n
    for j in range(n):
        incValinhos[j] = obsValinhos.Q[j] - \
            ((1 - solution[8]) * newQ[j]) + obsValinhos.C[j]
    ...
    desp = [0] * n
    for j in range(n):
        desp[j] = (obsValinhos.Q[j] + obsValinhos.C[j] -
                   incValinhos[j]) / (1 - paramsValinhos['Cp'])
    ...
    reserv = [0] * n
    for j in range(n):
        reserv[j] = (obsAtibaia.Q[j] + obsAtibaia.C[j] -
                     incAtibaia[j]) / (1 - paramsAtibaia['Cp'])
    ...
    # Armazenamento para plotagem
    upAA = UpstreamFORK(solution[0], solution[1], solution[2], 24.0, list(
